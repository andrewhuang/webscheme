...
public class Template {
	...
	/**
	 * Represents a chunk of a template, either a text run or a symbol that
	...
	public Template(String templateText) {
		...
		/*
		 * Plow through the string, scooping up content runs and symbols
		 * (stripping out open and close delimiters) and putting them into the
		 * cache vector, templateChunks
		 */
		...
		while (pos < length) {
			if (pos == -1) { /* No more open tags */
				templateChunks.add(new TemplateChunk(templateText
						.substring(start), false));
				break;
			}
			if (pos >= start) { /* Preceeding text run */
				end = pos;
				templateChunks.add(new TemplateChunk(templateText.substring(
						start, end), false));
				start = end + 1;
			}
			/* Find the close tag */
			...
			if (pos == -1) { /* No close tag */
				...
				TemplateChunk symbolChunk = new TemplateChunk(templateText
						.substring(start, end), true);
				// add symbol chunk
	...
	public void dump() {
		...
		for (int i = 0; i < cacheLength; i += 1) {
			...
			System.out.println("[" + i + "] = \"" + chunk.data + "\" "
					+ (chunk.isSymbol == true ? "[symbol]" : "[text]"));
		}
	}

	/**
	 * Substitutes the tag symbols in the given template with values obtained
	 * from the data object, or returns the original template text if the data
	 * object is null.
	 * 
	 * @param templateName
	 *            the name of the template
	 * @param data
	 *            the data object to substitute values from
	 * @return the substituted text
	 */
	public final String fill(Map data) {
		/* Lazy guess of minimum buffer size */
		StringBuffer resultBuffer = new StringBuffer(templateChunks.size() * 20);
